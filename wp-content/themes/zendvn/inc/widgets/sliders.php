<?php
class Zendvn_Theme_Widget_Sliders extends WP_Widget {

	//private $_cache_name = 'zendvn_mp_wg_last_post_caching';
	public function __construct() {
		//echo '<br/>' . __METHOD__;
		$id_base = 'zendvn-theme-widget-sliders';
		$name	= 'Zendvn Sliders';
		$widget_options = array(
					'classname' => 'widget_wpex_slider_widget',
					'description' => 'Hiển thị Sliders '
				);
	...
	public function widget( $args, $instance ) {
		...
		if($showFlag == true){
			...
			if($show_type == 'sidebar' ){
				require ZENDVN_THEME_WIDGET_DIR . '/html/sidebar.php';
			}else if($show_type == 'top_content'){
				require ZENDVN_THEME_WIDGET_DIR . '/html/top_content.php';
			}else if($show_type == 'bottom_content'){
				require ZENDVN_THEME_WIDGET_DIR . '/html/bottom_content.php';
			}
			wp_reset_postdata();
	...
	private function get_new_img_url($imgUrl, $width = 0, $heigt = 0 ,	$suffixes = '-zendvn-slider-'){
